...
wdamp = 0.9; %1.3; %1.3; %0.99; 1.1;      % Damping Ratio of Inetia Coefficient
...
for j=1:size(uv,1)
    if uv(1,1) == 1
        eval(['ROI',num2str(j),'=DATA(DATA(:,3)==',num2str(j),',:);']); %ROI1 = DATA(DATA(:,3)==1,:);
        ...
        %if uv(1,1) == -1
            eval(['ROI0=DATA(DATA(:,3)==-1,:);']); %ROI0 = DATA(DATA(:,3)==-1,:);
        %else
            %ROI0=[];
            eval(['ROI',num2str(j-1),'=DATA(DATA(:,3)==',num2str(j-1),',:);']);            
